...
function fetchBatchOfRandomMosaicEntries()
{
	$mosaicEntries = array();
	
	$dbh = newPDOConnection();
	
	$qString = "SELECT wpm.`post_id` as 'post_id', pt.`guid` AS 'orig_img_url', pm.`meta_value` AS 'meta_value' FROM wp_posts_mosaic AS wpm LEFT JOIN wp_posts AS pt ON wpm.`mosaic_post_id` = pt.`ID` LEFT JOIN wp_postmeta AS pm ON pt.`ID` = pm.`post_id` WHERE pm.`meta_key`='_wp_attachment_metadata' ORDER BY RAND() LIMIT 20;";
