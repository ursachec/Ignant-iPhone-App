//
//  Constants_Google_Analytics.m
//  IgnantApp
//
//  Created by Claudiu-Vlad Ursache on 13.07.12.
//  Copyright (c) 2012 c.v.ursache. All rights reserved.
//

#import "Constants_Google_Analytics.h"

NSInteger const kGANDispatchPeriodSec = 10;
NSString* const kAnalyticsAccountId = @"UA-33084223-1";


//google analytics paveviews
NSString* const kGAPVAppEntryPoint = @"/app_entry_point";
NSString* const kGAPVMosaicView = @"/mosaic_view";

NSString* const kGAPVAboutView = @"/about_view";
NSString* const kGAPVTumblrView = @"/tumblr_view";
NSString* const kGAPVCategoriesView = @"/categories_view";
NSString* const kGAPVCategoryView = @"/category_view/%@";
NSString* const kGAPVArticleDetailView = @"/article_detail_view/%@";
NSString* const kGAPVArticleImageSlideshowView = @"/article_image_slideshow_view/%@";
NSString* const kGAPVFavoritesView = @"/favorites_view/%d";
NSString* const kGAPVContactView = @"/contact_view";


//events



